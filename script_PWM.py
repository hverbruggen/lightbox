...
parser.add_argument("mode", help="Run mode. Default is 'loop'. To calibrate the white light, use 'calib_whitePWM'", default="loop")
...
# set up white light as PWM
whitePin = 18
whiteControl = PWMLED(whitePin)
white = .6
#for i in range(10):
#	whiteControl.value = i/10
#	if verbose: print("value: "+str(i/10))
#	time.sleep(1)

# this block allows setting up white/nir strips (they are controlled as if they were r & g channels of rgb strip)
...
# calibration mode
if (args.mode == "calib_whitePWM"):
	print("calibrating white LED via PWM\nwill loop through values of 0.1-1.0")
	for i in range(10):
		whiteControl.value = i/10
		print("value is now "+str(i/10)+" -- make light measurement and press Enter to continue to next value")
		input()
	quit()
...
def update_settings_and_log():  # updates the LED and fan settings, reads out the temperature and humidity, and writes the values to the log file
	...
	#print(str(data[time_code]['white']))
	white = round(float(data[time_code]['white']),2)
	...
	if verbose: print("    fan speed: "+str(fanSpeed))
	fanControl.value = fanSpeed
	if verbose: print("    white (PWM): "+str(white))
	whiteControl.value = white
	if verbose: print("    red/green/blue: "+str(red)+"/"+str(green)+"/"+str(blue))
	rgbStrip.setcolourrgb(red,green,blue)
#	print("setting nir (white via RGB) to "+str(nir))
#	wnStrip.setcolourrgb(0,nir,0)
#	print("setting whitePWM to "+str(white))
#	whitePWM.ChangeDutyCycle(white)
	if verbose: print("    done")
